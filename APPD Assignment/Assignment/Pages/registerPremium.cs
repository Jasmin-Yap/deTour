...
{
    public partial class registerPremium : Form
    {
        ...
        private void registerPremium_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        private void registerBtn_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string cardno = cardNoText.Text.Trim();
                string cardname = cardNameText.Text.Trim();
                string cardcvv = cardCVVText.Text.Trim();
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        else
                                                        {
                                                            if (Regex.IsMatch(email, pattern))
                                                            {
                                                                ...
                                                                if (cardno.Equals("") || cardname.Equals("") || cardcvv.Equals(""))
                                                                {
                                                                    MessageBox.Show("Please enter your payment information!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error); //error if card is blank
                                                                }
                                                                else
                                                                {
                                                                    if (repeat == false)
                                                                    {
                                                                        UserData.createUser('P', salution, name, username, password, address, Convert.ToInt32(zipCd), passNo, passED, Convert.ToInt32(phoneNo), email, 0.1);
                                                                        UserData.InsertPremiumPayment(username, cardno, cardname, Convert.ToDateTime(cardEDDTP.Text));
